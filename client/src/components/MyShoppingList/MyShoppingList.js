        ...
        <div>
          ...
          <ul>
            {this.state.MyShoppingList.map((item, index) => {
              return <div className="item-shopping-list"><li>{item}</li><i class="fas fa-trash-alt" style={{ fontSize: 15,color: "#42A9B6"}} onClick={e=>this.removeItem(index)} key={index}></i></div> ;
            })}
          </ul>
        </div>
